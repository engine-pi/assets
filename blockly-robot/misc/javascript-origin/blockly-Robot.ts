// https://raw.githubusercontent.com/France-ioi/bebras-modules/master/pemFioi/blocklyRobot_lib-1.1.js
// Konvertiert nach Typescript, um die Code besser verstehen zu können.
...
var initArray = function (n: number, elem: any): any[] {
  var res: any[] = [];
  for (var i = 0; i < n; i++) {
    res.push(elem);
  }
...
var getContext = function (display, infos, curLevel) {
  ...
  var contextStrings = {
    none: {
      fr: {
        ...
        description: {
          ...
          right:
            "tournerDroite() fait se tourner le robot d'un quart de tour à droite",
    ...
    castle: {
      fr: {
        ...
        messages: {
          ...
          failureContainersFilled:
            "Votre robot n'a pas mis du bois dans toutes les cheminées.",
          failureContainersFilledLess:
            "Votre robot n'a pas mis du bois dans toutes les cheminées..",
          failureContainersFilledBag:
            "Votre robot doit déposer le bois dans la cheminée.",
          failureDropOutside:
          ...
          failureDropPlatform: "Il y a déjà une plateforme ici !",
          emptyBag:
            "Le robot essaie de déposer du bois alors qu'il n'en transporte pas !",
        },
      },
    ...
    flowers: {
      fr: {
        ...
        description: {
          ...
          onContainer:
            "terreSurCase() indique s'il y a de la terre sur la case du robot",
          obstacleInFront:
            "fleurDevant() indique s'il y a une fleur sur la case devant le robot",
        },
        messages: {
    ...
    fishing: {
      fr: {
        ...
        description: {
          withdrawObject:
            "prendrePoissons() prend les poissons qui se trouvent sur la case",
          dropObject:
            "deposerPoissons() dépose sur la case les poissons transportés",
          onObject:
            "surPoissons() indique s'il y a un ou des poissons sur la case",
          onContainer: "surIle() indique s'il y a une île sur la case",
          nbWithdrawables:
            "nbPoissonsSur() indique combien de poissons sont sur la case",
          containerSize:
            "nbPoissonsCommandes() indique il faut livrer de poissons sur l'île",
          dropNum: "deposer(nbPoissons) dépose nbPoissons poissons sur la case",
          withdrawNum:
            "prendre(nbPoissons) prend nbPoissons poissons sur la case",
        },
        messages: {
          ...
          successContainersFilled:
            "Bravo, votre robot a apporté du poisson sur toutes les îles !",
          failureContainersFilled: "Votre robot a remis du poisson à l'eau !",
          failureContainersFilledLess:
            "Il reste encore au moins un poisson à apporter sur une île.",
          failureContainersFilledBag:
            "Il faut apporter les poissons sur une île !",
          ...
          failureDropObject:
            "Votre robot essaie de déposer trop de poissons sur cette île.",
        },
      },
    ...
    marbles: {
      fr: {
        ...
        description: {
          withdrawObject:
            "ramasserBille() ramasse la bille de la case du robot",
          dropObject:
            "deposerBille() dépose la bille transportée sur la case du robot",
          onObject: "surBille() indique une bille est sur la case du robot",
          onContainer:
            "surTrou() indique s'il y a un trou sur la case du robot",
        },
        messages: {
    ...
    packages: {
      fr: {
        ...
        description: {
          withdrawObject:
            "ramasserLivre() ramasse le livre qui se trouve sur la case",
          ...
          nbWithdrawables:
            "nbLivresSur() indique combien de livres sont sur la case",
          containerSize:
            "nbLivresCommandes() indique combien de livres sont commandés pour ce carton",
        ...
        messages: {
          ...
          successContainersFilled:
            "Bravo, tous les cartons sont pleins. Les colis vont pouvoir être livrés !",
          failureContainersFilled:
            "Tous les livres n'ont pas été mis en carton.",
          failureContainersFilledLess:
            "Il y a au moins un carton qui n'est pas plein.",
          failureContainersFilledBag: "Votre robot a ramassé trop de livres.",
          failureDropObject:
            "Ce colis est déjà plein. Votre robot essaie de déposer trop de livres.",
    ...
    pixelArt: {
      fr: {
        messages: {
          successContainersFilled:
            "Bravo, votre dessin est identique au modèle. Quel artiste !",
          failureContainersFilled:
            "Regardez bien le modèle, votre dessin n'est pas identique.",
          failureContainersFilledLess: "Le dessin n'est pas fini !",
          failureDropObject:
            "Le robot n'a pas peint la case de la bonne couleur.",
        },
      },
    },
    rocket: {
    ...
    sokoban: {
      fr: {
        ...
        description: {
          onContainer:
            "surCaseMarquee() indique si le robot se trouve sur une case marquée",
          pushableInFront:
            "caisseDevant() indique si le robot est juste devant une caisse",
          pushObject:
            "pousserCaisse() avance le robot en poussant la caisse qui est devant",
        },
        messages: {
          ...
          obstacle:
            "Le robot essaie de foncer dans un mur ou dans une caisse !",
        },
      },
    ...
    veterinary: {
      fr: {
        ...
        description: {
          withdrawObject:
            "ramasserBois() ramasse le bois qui se trouve sur la case",
          ...
          nbWithdrawables:
            "nbBuchesSur() indique combien de bûches sont sur la case",
          containerSize:
            "nbBuchesADeposer() indique combien de bûches sont demandées sur la case",
        ...
        messages: {
          ...
          successContainersFilled:
            "Bravo, tous les castors ont du bois. Ils remercient votre robot !",
  ...
  function getImgPath(url: string) {
  ...
  infos.newBlocks.push({
    ...
    func: function (callback) {
      ...
      this.callCallback(
        callback,
        this.getItemsOn(robot.row, robot.col, function (obj: Item) {
          return obj.isWithdrawable === true;
        }).length
      );
    },
  ...
  infos.newBlocks.push({
    ...
    func: function (callback) {
      var robot = this.getRobot();
      var containers = this.getItemsOn(
        robot.row,
        robot.col,
        function (obj: Item) {
          return obj.isContainer === true;
  ...
  infos.newBlocks.push({
    ...
    func: function (callback) {
      this.callCallback(
        callback,
        this.isInFront(function (obj: Item) {
  ...
  infos.newBlocks.push({
    ...
    block: {
      ...
      params: [null],
      blocklyXml:
        "<block type='withdrawNum_noShadow'>" +
        "  <value name='PARAM_0'>" +
  ...
  infos.newBlocks.push({
    ...
    func: function (callback) {
      ...
      if (
        this.getItemsOn(coords.row, coords.col, function (item: Item) {
  ...
  if (infos.languageStrings != undefined) {
    context.importLanguageStrings(
      infos.languageStrings.blocklyRobot_lib,
      strings
    );
  }
  ...
  var resetItemsZOrder = function (row, col) {
    ...
    cellItems.sort(function (itemA: Item, itemB: Item) {
  ...
  var redisplayItem = function (item: Item, resetZOrder?: boolean) {
    ...
    if (item.img) {
      ...
      var value;
      if (typeof item.value == "function") value = item.value(item);
      else value = item.value;

      item.element = paper
        .text(x + (item.side * scale) / 2, y + (item.side * scale) / 2, value)
        .attr({
  ...
  context.updateScale = function () {
    ...
    if (context.nbCols && context.nbRows) {
      var marginAsCols =
        (infos.leftMargin + infos.rightMargin) / infos.cellSide;
      var marginAsRows =
        (infos.topMargin + infos.bottomMargin) / infos.cellSide;
    ...
    if (infos.showCardinals) {
      var middleX =
        (infos.leftMargin + (infos.cellSide * context.nbCols) / 2) * scale;
      var middleY =
        (infos.topMargin + (infos.cellSide * context.nbRows) / 2) * scale;
      cardLabels[0]
        .attr({
          x: middleX,
          y:
            (infos.topMargin -
              (infos.showLabels ? infos.cellSide : 0) -
              infos.cellSide / 2) *
            scale,
        })
        .attr(textFontSize);
      cardLabels[1]
        .attr({
          x: middleX,
          y: paperHeight + (infos.cellSide / 2 - infos.bottomMargin) * scale,
        })
        .attr(textFontSize);
      cardLabels[2]
        .attr({
      ...
      cardLabels[3]
        .attr({
          x: paperWidth + (infos.cellSide / 2 - infos.rightMargin) * scale,
          y: middleY,
        })
        .attr(textFontSize);
    }
  ...
  context.advanceTime = function (epsilon) {
    ...
    for (var iTime = 0; iTime < epsilon; iTime++) {
      ...
      var robot = this.getRobot();
      if (
        this.hasOn(robot.row, robot.col, function (item: Item) {
  ...
  context.getRobot = function (): Item {
  ...
  context.hasOn = function (row: number, col: number, filter: Filter): boolean {
    for (var id in context.items) {
  ...
  context.setIndexes = function (): void {
    for (var id in context.items) {
      var item = context.items[id];
      item.index = id;
    }
  ...
  context.getItemsOn = function (row: number, col: number, filter: Filter): Item[] {
    if (filter === undefined) {
      filter = function (obj) {
        return true;
      };
  ...
  context.tryToBeOn = function (row: number, col: number) {
    ...
    if (
      context.hasOn(row, col, function (item: Item) {
        return item.isObstacle === true;
      })
    ) {
      if (infos.ignoreInvalidMoves) return false;
  ...
  context.moveProjectile = function (item: Item) {
    if (!context.isInGrid(item.row + 1, item.col)) {
      context.destroy(item);
    }

    if (
      context.hasOn(item.row + 1, item.col, function (item: Item) {
        return item.isObstacle === true;
      })
    ) {
      context.destroy(item);
    ...
    if (containers.length != 0) {
      ...
      if (container.containerFilter != undefined) {
        if (
          ...
          context.infos.blockingFilter !== false
        ) {
          throw window.languageStrings.messages.failureDropObject;
          return;
        }
  ...
  context.drop = function (count, coords, filter: Filter) {
    if (count === undefined) {
      count = 1;
    }
    if (filter === undefined) {
  ...
  context.forward = function (callback) {
    ...
    if (ttbo === true) {
      ...
      context.moveRobot(
        robot.row + (coords.row - robot.row) / 4,
        robot.col + (coords.col - robot.col) / 4,
        robot.dir
      );
      throw ttbo;
    }
  ...
  checkContainersFilled: function (context, lastTurn) {
    ...
    if (context.infos.maxMoves != undefined) {
      if (context.nbMoves > context.infos.maxMoves) {
        context.success = false;
        throw (
          window.languageStrings.messages.failureTooManyMoves +
          " : " +
          context.nbMoves
        );
      }
...
var robotEndFunctionGenerator = {
  allFilteredPicked: function (filter: Filter) {
...
var contextParams = {
  ...
  cards: {
    ...
    itemTypes: {
      ...
      triangleQuadrille: {
        img: "card_triangleQuadrille.png",
        side: 60,
        isWithdrawable: true,
        isTriangle: true,
        isQuadrille: true,
  ...
  fishing: {
    ...
    itemTypes: {
      robot: {
        ...
        isRobot: true,
        zOrder: 4,
        customDisplay: function (obj: Item) {
          if (context.bag.length != 0) obj.img = "buoy_robot_fishes.png";
          else obj.img = "buoy_robot.png";
        },
      },
      island: {
        num: 2,
        ...
        offsetY: 0,
        zOrder: 0,
        containerFilter: function (obj: Item) {
          return obj.isWithdrawable === true;
        },
      },
      fishes: {
      ...
      fishes: {
        ...
        zOrder: 1,
        canBeOutside: true,
        customDisplay: function (obj) {
          if (
            context.hasOn(obj.row, obj.col, function (item: Item) {
              return item.num == 2;
            })
          )
            obj.offsetX = 0;
      ...
      count_fishes: {
        num: 5,
        value: function (obj: Item) {
          return context.getItemsOn(obj.row, obj.col, function (item: Item) {
  ...
  packages: {
    ...
    itemTypes: {
      robot: {
        ...
        zOrder: 4,
        customDisplay: function (obj) {
          if (context.bag.length != 0) obj.img = "package_robot_book.png";
          else obj.img = "package_robot.png";
        },
      },
      box: {
        num: 2,
  ...
  paint: {
    ...
    itemTypes: {
      ...
      marker: {
        ...
        side: 60,
        isContainer: true,
        containerFilter: function (item: Item) {
